...
*/
	...
	SynchronizationsAnalyzer::SynchronizationsAnalyzer (FrameID first, FrameID last)
	{
		SetRange( first, last );
	}
...
=================================================
*/
	void SynchronizationsAnalyzer::SetRange (FrameID first, FrameID last)
	{
		ASSERT( first < last );
...
*/
	...
	{
		if ( frameId + _frameCmpBias > _lastFrame )
			return;

		ASSERT( _uidCounter < (1ull << 63) );
	...
	bool SynchronizationsAnalyzer::_OnDeviceWaitIdle (const TraceRange::Iterator &pos, ResourceID id, FrameID frameId)
	{
		...
		for (auto& q : _queues)
		{
			...
			if ( q_info.deviceId == id )
			{
				...
				bm.input.push_back({ qbm.uid, EConnectionFlags::HostWait });
				qbm.output.push_back({ bm.uid, EConnectionFlags::HostWait });

				CHECK( _ReleaseUnusedQueueData( q.first, frameId ));
			}
		}
		return true;
	}
...
*/
	bool SynchronizationsAnalyzer::_OnWaitForFences (const TraceRange::Iterator &pos, ResourceID id, FrameID frameId)
	{
		...
		for (uint i = 0; i < packet.fenceCount; ++i)
		{
			...
			UID		uid = 0;
			CHECK_ERR( _FindSignaledFence( dev->second.back(), ResourceID(packet.pFences[i]), frameId, bm.uid, OUT uid ));
			bm.input.push_back({ uid, EConnectionFlags::HostWait });
		}
...
*/
	bool SynchronizationsAnalyzer::_OnQueueSubmit (const TraceRange::Iterator &pos, ResourceID id, FrameID frameId)
	{
		...
		// add host sync
		{
			...
			bm.frameId	= frameId;
			bm.threadId	= ThreadID(pos->thread_id);
			bm.data		= HostQueueSubmit{ ResourceID(packet.queue), ResourceID(packet.fence), fence_uid };
				
			// add dependency on execution start
			for (UID i = first_uid; i < fence_uid; i += 2) {
				bm.output.push_back({ i, EConnectionFlags::Execution | EConnectionFlags::Strong });
			}
		}
		...
		if ( packet.fence )
		{
			...
			// add queue sync
			{
				...
				for (UID i = first_uid+1; i < fence_uid; i += 2) {
					bm.input.push_back({ i, EConnectionFlags::Execution });
				}
			}
		}
		return true;
													...
													UID uid, UID fenceUID, UID hostSubmitUID)
	{
		// start command execution
		{
			...
			for (uint j = 0; j < src.waitSemaphoreCount; ++j)
			{
				...
				CHECK_ERR( _WaitSemaphore( ResourceID(src.pWaitSemaphores[j]),
										   [&bm](ResourceID, UID uid) { bm.input.push_back({ uid, EConnectionFlags::Wait }); } ));
			}
			...
			bm.input.push_back({ hostSubmitUID, EConnectionFlags::Execution | EConnectionFlags::Strong });
		...
		// finish execution
		{
			...
			if ( fenceUID )
				bm.output.push_back({ fenceUID, EConnectionFlags::Execution });
		}
		return true;
	}
...
*/
	bool SynchronizationsAnalyzer::_OnQueueWaitIdle (const TraceRange::Iterator &pos, ResourceID id, FrameID frameId)
	{
		...
		// add queue sync
		{
			...
			bm.output.push_back({ bm.uid + 1, EConnectionFlags::HostWait });
		}
		...
		// add host sync
		{
			...
			bm.input.push_back({ bm.uid - 1, EConnectionFlags::HostWait });
		}

		CHECK( _ReleaseUnusedQueueData( id, frameId ));
		return true;
	}
	...
	bool SynchronizationsAnalyzer::_OnCmdPipelineBarrier (const TraceRange::Iterator &pos, CommandBufferState &state) const
	{
		...
		if ( packet.pMemoryBarriers ) {
			CHECK_ERR( _ProcessMemoryBarriers( packet.srcStageMask, packet.dstStageMask, packet.dependencyFlags,
											   packet.memoryBarrierCount, packet.pMemoryBarriers, pos->packet_id, state ));
		}
		...
		if ( packet.pImageMemoryBarriers ) {
			CHECK_ERR( _ProcessImageBarriers( packet.srcStageMask, packet.dstStageMask, packet.dependencyFlags,
											  packet.imageMemoryBarrierCount, packet.pImageMemoryBarriers, pos->packet_id, state ));
		}
		...
		if ( packet.pBufferMemoryBarriers ) {
			CHECK_ERR( _ProcessBufferBarriers( packet.srcStageMask, packet.dstStageMask, packet.dependencyFlags,
											   packet.bufferMemoryBarrierCount, packet.pBufferMemoryBarriers, pos->packet_id, state ));
		}
		return true;
	}
													  ...
													  TraceRange::Bookmark pos, PacketID packetId, CommandBufferState &state) const
	{
		...
		if ( dependency.srcSubpass != VK_SUBPASS_EXTERNAL and sync.subpassIndex == dependency.dstSubpass )
		{
			...
			for (auto iter = barriers.rbegin(), end = barriers.rend(); iter != end; ++iter)
			{
				...
				iter->output.push_back({ sync.uid, EConnectionFlags::Barrier });
				iter->outputComplete = true;

				sync.input.push_back({ iter->uid, EConnectionFlags::Barrier });
	...
	void SynchronizationsAnalyzer::_MergeSyncQueues (INOUT SyncQueue &dst, const SyncQueue &src) const
	{
		...
		for (auto& img : src.imageSync)
		{
			...
			for (auto imgbar : img.second)
			{
				...
				for (auto& dep : imgbar.input)	{ dep.uid = Remap( dep.uid ); }
				for (auto& dep : imgbar.output)	{ dep.uid = Remap( dep.uid ); }
		...
		for (auto& buf : src.bufferSync)
		{
			...
			for (auto bufbar : buf.second)
			{
				...
				for (auto& dep : bufbar.input)	{ dep.uid = Remap( dep.uid ); }
				for (auto& dep : bufbar.output)	{ dep.uid = Remap( dep.uid ); }
		...
		for (auto glob : src.globalSync)
		{
			...
			for (auto& dep : glob.input)	{ dep.uid = Remap( dep.uid ); }
			for (auto& dep : glob.output)	{ dep.uid = Remap( dep.uid ); }
...
=================================================
*/
	bool SynchronizationsAnalyzer::_ProcessImage (const TraceRange::Iterator &pos, ResourceID imageId, FrameID, EResOp)
	{
		switch ( pos->packet_id )
		{
...
=================================================
*/
	bool SynchronizationsAnalyzer::_FindSignaledFence (DeviceInfo_t &dev, ResourceID fenceId, FrameID frameId, UID currUID, OUT UID &uid)
	{
		uid = 0;
		...
		for (auto iter = dev.bookmarks.rbegin(), end = dev.bookmarks.rend(); iter != end; ++iter)
		{
			...
			Visit( iter->data,
				...
				[&] (const HostQueueSubmit &cmd) {
					if ( fenceId == cmd.fence ) {
						status	= 3;
						uid		= cmd.fenceUID;

						auto*	queue = this->_queues.FindIn( cmd.queueId, iter->pos );
						CHECK_ERR( queue, void());
						CHECK( _BuildCommandDependencies( cmd.fenceUID, *queue ));
						CHECK( _ReleaseUnusedQueueData( cmd.queueId, frameId ));
					}
				},
			...
			switch ( status ) {
				...
				case 1 :
					iter->output.push_back({ currUID, EConnectionFlags::HostWait });
					uid = iter->uid;
					return true;
...
/*
=================================================
	_ReleaseUnusedQueueData
=================================================
*/
	bool SynchronizationsAnalyzer::_ReleaseUnusedQueueData (ResourceID queueId, FrameID frameId)
	{
		if ( frameId + _frameCmpBias < _firstFrame )
		{
			for (auto& q : _queues)
			{
				if ( q.first != queueId )
				...
				for (auto& inst : q.second) {
					if ( inst.bookmarks.size() > 1 )
						inst.bookmarks.erase( inst.bookmarks.begin()+1, inst.bookmarks.end() );
				}
			}

			/*for (auto& sw : _swapchains) {
				for (auto& inst : sw.second) {
					if ( inst.bookmarks.size() > 1 )
						inst.bookmarks.erase( inst.bookmarks.begin()+1, inst.bookmarks.end() );
				}
			}
	...
	static void BuildImageDependencies (SynchronizationsAnalyzer::ImageBarriers_t::value_type &img, UID finalUID, INOUT ImageDependencies &deps)
	{
		...
		for	(auto iter = img.second.rbegin(), end = img.second.rend(); iter != end; ++iter)
		{
			...
			if ( curr == deps.states.end() )
			{
				...
				curr->second->output.push_back({ finalUID, EConnectionFlags::Execution });
			}
			else
			...
			{
				iter->output.push_back({ curr->second->uid, EConnectionFlags::Barrier });
				curr->second->input.push_back({ iter->uid, EConnectionFlags::Barrier });
...
*/
	...
	static void BuildGlobalDependencies (Array<SynchronizationsAnalyzer::SyncItem> &syncs, UID finalUID, INOUT GlobalDependencies &deps)
	{
		...
		{
			if ( deps.last == null ) {
				iter->output.push_back({ finalUID, EConnectionFlags::Execution });
			}else{
				deps.last->input.push_back({ iter->uid, EConnectionFlags::Barrier });
				iter->output.push_back({ deps.last->uid, EConnectionFlags::Barrier });
			}
				...
				[&bm_iter, &deps, finalUID] (QueueExecutionStart &cmd)
				{
					for (auto q_iter = cmd.syncQueues.rbegin(), q_end = cmd.syncQueues.rend(); q_iter != q_end; ++q_iter)
					{
						for (auto& img : q_iter->imageSync) {
							if ( not img.second.back().output.empty() ) continue;
							BuildImageDependencies( img, finalUID, INOUT deps.images );
						}
						for (auto& buf : q_iter->bufferSync) {
							if ( not buf.second.back().output.empty() ) continue;
					...
					if ( not cmd.waitSemaphores.empty() )
					{
						for (auto& img : deps.images.states) {
							img.second->input.push_back({ bm_iter->uid, EConnectionFlags::Execution });
						}
						for (auto& buf : deps.buffers.states) {
							buf.second->input.push_back({ bm_iter->uid, EConnectionFlags::Execution });
						}
						if ( deps.global.last ) {
							deps.global.last->input.push_back({ bm_iter->uid, EConnectionFlags::Execution });
						}
						deps = {};
					}
				},
		...
		for (auto bm_iter = queue.bookmarks.rbegin(), bm_end = queue.bookmarks.rend(); bm_iter != bm_end; ++bm_iter)
		{
			...
			Visit( bm_iter->data,
				[&bm_iter, &finished, &last] (const QueueExecutionFinish &) {
					finished = true;
					bm_iter->output.push_back({ last.uid, EConnectionFlags::Execution | EConnectionFlags::Strong });
					last.input.push_back({ bm_iter->uid, EConnectionFlags::Execution | EConnectionFlags::Strong });
				},
				[&bm_iter, &finished, &last] (const QueueFenceSignal &) {
					finished = true;
					bm_iter->output.push_back({ last.uid, EConnectionFlags::Wait });
					last.input.push_back({ bm_iter->uid, EConnectionFlags::Wait });
				},
