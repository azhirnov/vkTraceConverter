...
*/
	bool DeviceAnalyzer::_OnGetDeviceQueue (const TraceRange::Iterator &pos, LogicalDeviceInfo &dev)
	{
		...
		{
			if ( q.queueFamilyIndex == packet.queueFamilyIndex )
			{
				CHECK_ERR( packet.queueIndex < q.instances.size() );
				q.instances[ packet.queueIndex ].id = ResourceID(*packet.pQueue);
				return true;
			}
		}

		RETURN_ERR( "queue not found" );
	}
...
*/
	bool DeviceAnalyzer::_OnGetDeviceQueue2 (const TraceRange::Iterator &pos, LogicalDeviceInfo &dev)
	{
		...
		for (auto& q : dev.queues)
		{
			if ( q.queueFamilyIndex == packet.pQueueInfo->queueFamilyIndex )
			{
				CHECK_ERR( packet.pQueueInfo->queueIndex < q.instances.size() );
				q.instances[ packet.pQueueInfo->queueIndex ].id = ResourceID(*packet.pQueue);
				return true;
			}
		}
