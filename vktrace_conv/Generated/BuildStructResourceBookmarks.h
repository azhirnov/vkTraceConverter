...
switch ( header->sType )
{
	...
	case VK_STRUCTURE_TYPE_MEMORY_GET_FD_INFO_KHR : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT, ResourceID((value).memory), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_FENCE_GET_WIN32_HANDLE_INFO_KHR : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_FENCE_EXT, ResourceID((value).fence), iter, frame_id );
		#endif
		break;
	}
	...
	case VK_STRUCTURE_TYPE_DEDICATED_ALLOCATION_MEMORY_ALLOCATE_INFO_NV : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT, ResourceID((value).image), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, ResourceID((value).buffer), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_BIND_SPARSE_INFO : {
		...
		for (uint a = 0; (value).pWaitSemaphores and a < (value).waitSemaphoreCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SEMAPHORE_EXT, ResourceID((value).pWaitSemaphores[a]), iter, frame_id );
		}
		for (uint a = 0; (value).pBufferBinds and a < (value).bufferBindCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, ResourceID(((value).pBufferBinds[a]).buffer), iter, frame_id );
			for (uint b = 0; ((value).pBufferBinds[a]).pBinds and b < ((value).pBufferBinds[a]).bindCount; ++b) {
				_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT, ResourceID((((value).pBufferBinds[a]).pBinds[b]).memory), iter, frame_id );
			}
		}
		for (uint a = 0; (value).pImageOpaqueBinds and a < (value).imageOpaqueBindCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT, ResourceID(((value).pImageOpaqueBinds[a]).image), iter, frame_id );
			for (uint b = 0; ((value).pImageOpaqueBinds[a]).pBinds and b < ((value).pImageOpaqueBinds[a]).bindCount; ++b) {
				_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT, ResourceID((((value).pImageOpaqueBinds[a]).pBinds[b]).memory), iter, frame_id );
			}
		}
		for (uint a = 0; (value).pImageBinds and a < (value).imageBindCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT, ResourceID(((value).pImageBinds[a]).image), iter, frame_id );
			for (uint b = 0; ((value).pImageBinds[a]).pBinds and b < ((value).pImageBinds[a]).bindCount; ++b) {
				_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT, ResourceID((((value).pImageBinds[a]).pBinds[b]).memory), iter, frame_id );
			}
		}
		for (uint a = 0; (value).pSignalSemaphores and a < (value).signalSemaphoreCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SEMAPHORE_EXT, ResourceID((value).pSignalSemaphores[a]), iter, frame_id );
		}
		break;
	}
	...
	case VK_STRUCTURE_TYPE_SHADER_MODULE_VALIDATION_CACHE_CREATE_INFO_EXT : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_VALIDATION_CACHE_EXT_EXT, ResourceID((value).validationCache), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_CMD_RESERVE_SPACE_FOR_COMMANDS_INFO_NVX : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_OBJECT_TABLE_NVX_EXT, ResourceID((value).objectTable), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_INDIRECT_COMMANDS_LAYOUT_NVX_EXT, ResourceID((value).indirectCommandsLayout), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_IMPORT_SEMAPHORE_WIN32_HANDLE_INFO_KHR : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SEMAPHORE_EXT, ResourceID((value).semaphore), iter, frame_id );
		#endif
		break;
	}
	...
	case VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO : {
		...
		for (uint a = 0; (value).pBindings and a < (value).bindingCount; ++a) {
			for (uint b = 0; ((value).pBindings[a]).pImmutableSamplers and b < ((value).pBindings[a]).descriptorCount; ++b) {
				_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SAMPLER_EXT, ResourceID(((value).pBindings[a]).pImmutableSamplers[b]), iter, frame_id );
			}
		}
		break;
	}
	...
	case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SURFACE_INFO_2_KHR : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SURFACE_KHR_EXT, ResourceID((value).surface), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_SUBMIT_INFO : {
		...
		for (uint a = 0; (value).pCommandBuffers and a < (value).commandBufferCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT, ResourceID((value).pCommandBuffers[a]), iter, frame_id );
		}
		for (uint a = 0; (value).pSignalSemaphores and a < (value).signalSemaphoreCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SEMAPHORE_EXT, ResourceID((value).pSignalSemaphores[a]), iter, frame_id );
		}
	...
	case VK_STRUCTURE_TYPE_ACQUIRE_NEXT_IMAGE_INFO_KHR : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SWAPCHAIN_KHR_EXT, ResourceID((value).swapchain), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SEMAPHORE_EXT, ResourceID((value).semaphore), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_FENCE_EXT, ResourceID((value).fence), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_RENDER_PASS_EXT, ResourceID((value).renderPass), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_FRAMEBUFFER_EXT, ResourceID((value).framebuffer), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO : {
		...
		for (uint a = 0; (value).pSetLayouts and a < (value).setLayoutCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT_EXT, ResourceID((value).pSetLayouts[a]), iter, frame_id );
		}
		break;
	}
	...
	case VK_STRUCTURE_TYPE_DISPLAY_SURFACE_CREATE_INFO_KHR : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DISPLAY_MODE_KHR_EXT, ResourceID((value).displayMode), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_LAYOUT_EXT, ResourceID((value).layout), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_EXT, ResourceID((value).basePipelineHandle), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO : {
		...
		for (uint a = 0; (value).pStages and a < (value).stageCount; ++a) {
			...
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT, ResourceID(((value).pStages[a]).module), iter, frame_id );
		}
	...
	case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_GROUP_PROPERTIES : {
		...
		for (uint a = 0; (value).physicalDevices and a < VK_MAX_DEVICE_GROUP_SIZE; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_PHYSICAL_DEVICE_EXT, ResourceID((value).physicalDevices[a]), iter, frame_id );
		}
		break;
	}
	...
	case VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_POOL_EXT, ResourceID((value).commandPool), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_DEVICE_GROUP_DEVICE_CREATE_INFO : {
		...
		for (uint a = 0; (value).pPhysicalDevices and a < (value).physicalDeviceCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_PHYSICAL_DEVICE_EXT, ResourceID((value).pPhysicalDevices[a]), iter, frame_id );
		}
		break;
	}
	...
	case VK_STRUCTURE_TYPE_CMD_PROCESS_COMMANDS_INFO_NVX : {
		...
		for (uint a = 0; (value).pIndirectCommandsTokens and a < (value).indirectCommandsTokenCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, ResourceID(((value).pIndirectCommandsTokens[a]).buffer), iter, frame_id );
		}
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT, ResourceID((value).targetCommandBuffer), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, ResourceID((value).sequencesCountBuffer), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, ResourceID((value).sequencesIndexBuffer), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_COPY_DESCRIPTOR_SET : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT, ResourceID((value).srcSet), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT, ResourceID((value).dstSet), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO : {
		...
		for (uint a = 0; (value).pAttachments and a < (value).attachmentCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_VIEW_EXT, ResourceID((value).pAttachments[a]), iter, frame_id );
		}
		break;
	}
	...
	case VK_STRUCTURE_TYPE_WIN32_KEYED_MUTEX_ACQUIRE_RELEASE_INFO_KHR : {
		...
		for (uint a = 0; (value).pAcquireSyncs and a < (value).acquireCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT, ResourceID((value).pAcquireSyncs[a]), iter, frame_id );
		}
		for (uint a = 0; (value).pReleaseSyncs and a < (value).releaseCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT, ResourceID((value).pReleaseSyncs[a]), iter, frame_id );
		}
		#endif
		break;
	}
	...
	case VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT, ResourceID((value).module), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SURFACE_KHR_EXT, ResourceID((value).surface), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SWAPCHAIN_KHR_EXT, ResourceID((value).oldSwapchain), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_DISPLAY_PLANE_INFO_2_KHR : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DISPLAY_MODE_KHR_EXT, ResourceID((value).mode), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_SAMPLER_YCBCR_CONVERSION_INFO : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SAMPLER_YCBCR_CONVERSION_EXT, ResourceID((value).conversion), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_DESCRIPTOR_UPDATE_TEMPLATE_CREATE_INFO : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT_EXT, ResourceID((value).descriptorSetLayout), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_LAYOUT_EXT, ResourceID((value).pipelineLayout), iter, frame_id );
		break;
	}
	...
	case VK_STRUCTURE_TYPE_PRESENT_INFO_KHR : {
		...
		for (uint a = 0; (value).pSwapchains and a < (value).swapchainCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SWAPCHAIN_KHR_EXT, ResourceID((value).pSwapchains[a]), iter, frame_id );
		}
		break;
	}
	...
	case VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET : {
		...
		for (uint a = 0; (value).pImageInfo and a < (value).descriptorCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_SAMPLER_EXT, ResourceID(((value).pImageInfo[a]).sampler), iter, frame_id );
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_VIEW_EXT, ResourceID(((value).pImageInfo[a]).imageView), iter, frame_id );
		}
		for (uint a = 0; (value).pBufferInfo and a < (value).descriptorCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, ResourceID(((value).pBufferInfo[a]).buffer), iter, frame_id );
		}
		for (uint a = 0; (value).pTexelBufferView and a < (value).descriptorCount; ++a) {
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_VIEW_EXT, ResourceID((value).pTexelBufferView[a]), iter, frame_id );
		}
		break;
	}
	...
	case VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO : {
		...
		if ( (value).pInheritanceInfo ) {
			...
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_RENDER_PASS_EXT, ResourceID((*(value).pInheritanceInfo).renderPass), iter, frame_id );
			_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_FRAMEBUFFER_EXT, ResourceID((*(value).pInheritanceInfo).framebuffer), iter, frame_id );
		}
		break;
	}
	...
	case VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO : {
		...
		if ( (value).pNext )
			_AddStructBookmsrks( BitCast<VkBaseInStructure const*>((value).pNext), iter, frame_id );
		_AddResourceBookmark( VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_POOL_EXT, ResourceID((value).descriptorPool), iter, frame_id );
